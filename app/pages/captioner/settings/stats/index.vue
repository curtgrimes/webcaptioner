  ...
  <div>
    <div class="card card-body px-0">
      ...
      <div class="row mb-3 mb-sm-2 mt-2">
        ...
        <div class="col-sm-9">
          <div
            v-for="(phrase, index) in $store.state.captioner.transcript.cursorable"
            :key="index"
            class="d-inline"
          >
            <div
            ...
            >
              <div
                class="row m-0 px-1"
                :class="stable ? 'bg-success' : 'bg-danger text-white'"
              >{{word}}</div>
              <div
                class="row m-0 px-1"
                :class="stable ? 'text-muted' : 'text-danger'"
              >{{Date.now() - firstSeen}}</div>
            </div>
            <div
              v-if="index != $store.state.captioner.transcript.cursorable.length - 1"
              class="card card-body p-0 d-inline-block small bg-primary"
              style="overflow:hidden"
            >
          ...
          <table class="table table-sm mb-0">
            <tbody>
              <tr>
                ...
                <td>
                  <strong class="text-danger">{{$store.state.settings.locale.userDefault}}</strong>
                </td>
              </tr>
              <tr>
                <td>Closest match recognized by Web Speech API</td>
                <td>
                  <strong class="text-danger">{{closestMatchLocale}}</strong>
                </td>
              </tr>
              <tr>
                <td>Current user setting</td>
                <td>
                  <strong class="text-danger">{{$store.state.settings.locale.from}}</strong>
                </td>
              </tr>
              <tr>
                <td>window.navigator.languages</td>
                <td>
                  <strong class="text-danger">{{windowLocaleData.windowNavigatorLanguages}}</strong>
                </td>
              </tr>
              <tr>
                <td>window.navigator.userLanguage</td>
                <td>
                  <strong class="text-danger">{{windowLocaleData.windowNavigatorUserLanguage}}</strong>
                </td>
              </tr>
              <tr>
                <td>window.clientInformation.language</td>
                <td>
                  <strong class="text-danger">{{windowLocaleData.windowClientInformationLanguage}}</strong>
                </td>
              </tr>
              <tr>
                ...
                <td>
                  ...
                  <b-collapse id="localeList">
                    <ul
                      v-for="({code, nameEnglish}) in localesSortedByCode"
                      :key="code"
                      class="mb-0"
                    >
...
export default {
  ...
  computed: {
    windowLocaleData: function() {
      return {
        windowNavigatorLanguages: process.client
          ? window.navigator.languages
          : '',
        windowNavigatorUserLanguage: process.client
          ? window.navigator.userLanguage
          : '',
        windowClientInformationLanguage: process.client
    ...
    closestMatchLocale: function() {
      ...
      let closestMatch = this.locales.find((l) => {
        return (
          (l.code || '').toUpperCase() ==
          (this.$store.state.settings.locale.userDefault || '').toUpperCase()
        );
      });
    ...
    localesSortedByCode: function() {
      return this.locales.sort((a, b) => {
        return this.sortByProperty('code', a, b);
      });
    },
  },
  methods: {
