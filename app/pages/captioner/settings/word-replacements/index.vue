<template>
  <div class="settings-word-replacements-view">
    ...
    <div ref="wordReplacements">
      <div class="row mb-2" v-for="(wordReplacement, index) in wordReplacements" v-bind:key="index">
        <div :class="shouldShowDeleteButtons ? 'col-5' : 'col-6'">
          <input
            type="text"
            v-model="wordReplacement.from"
            @keyup="updateWordReplacement(wordReplacement, index)"
            class="form-control"
            :placeholder="$t('settings.wordReplacements.wordOrPhraseToReplaceSentenceCase')"
            v-bind:ref="index == wordReplacements.length - 1 ? 'lastWordReplacement' : ''"
          >
        </div>
        <div :class="shouldShowDeleteButtons ? 'col-5' : 'col-6'">
          <input
            type="text"
            v-model="wordReplacement.to"
            @keyup="updateWordReplacement(wordReplacement, index)"
            class="form-control"
            :placeholder="$t('settings.wordReplacements.replaceWithSentenceCase')"
          >
        </div>
    ...
    <button
      type="button"
      @click="addWordReplacement({from:'',to:''})"
      class="btn btn-success btn-sm mt-3"
    >
      <fa icon="plus-circle"/>
      {{$t('settings.wordReplacements.addAnother')}}
    </button>
  </div>
</template>
...
export default {
  ...
  methods: {
    addWordReplacement: function(wordReplacement) {
      ...
      setTimeout(() => {
        if (
          this.$refs.lastWordReplacement &&
          this.$refs.lastWordReplacement.length
        ) {
