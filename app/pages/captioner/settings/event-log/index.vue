...
export default {
  ...
  methods: {
    initEventLog: function() {
      ...
      this.eventLogStopTime = Date.now() + (1000 * 60 * this.loggingDurationMinutes);
      this.startTimer();

      this.eventLog = [];
      this.appendToEventLog({state: JSON.parse(JSON.stringify(this.$store.state))});
    ...
    startTimer: function() {
      let logTick = () => {
        ...
        if (now < this.eventLogStopTime) {
          ...
          this.timeRemainingMinutes = (Math.floor(minutesDecimal) + '').padStart(2,'0');
          this.timeRemainingSeconds = (Math.floor((minutesDecimal - this.timeRemainingMinutes) * 60) + '').padStart(2,'0');
        }
        else {
          this.stopLogging();
        }
