      ...
      <transition name="fade">
        <div v-if="rooms !== null">
          ...
          <div class="row px-4 small font-weight-bold mb-2" v-if="rooms.length">
            ...
            <div class="col-3">Viewers</div>
            <div class="col-3">Backlink</div>
            <div class="col-3">Expire Date</div>
          </div>
          <b-list-group>
            <b-list-group-item
              v-for="room in rooms"
            ...
            >
              <div class="row">
                <div class="col-3">{{room.id}}</div>
                <div class="col-3">{{room.subscriberCount}}</div>
                <div class="col-3" style="overflow:hidden">{{room.backlink}}</div>
                <div
                  class="col-3"
                  style="overflow:hidden"
                >{{new Date(room.expireDate).toISOString()}}</div>
              </div>
            </b-list-group-item>
          </b-list-group>
        </div>
        <div v-else-if="roomsError">
          Unable to get rooms.
          <a href="javascript:void(0)" @click="updateRooms()">Retry.</a>
        </div>
      </transition>
    </div>
  </div>
