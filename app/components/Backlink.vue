...
export default {
    ...
    methods: {
        updateRoomBacklink: async function() {
            ...
            try {
                const {backlink} = await this.$axios.$get('/api/rooms/' + this.roomId +'/backlink');
    ...
    computed: {
        backlinkStyle: function() {
            return {
                ...(this.backlink.colors ? {backgroundColor: this.backlink.colors.background, color: this.backlink.colors.text} : {}),
                ...(this.inline ?  {} : {position: 'absolute', top: '15px', right: '15px', zIndex: 1}),
            };
        },
    },
    watch: {
      backlinkHovering: function(backlinkHovering) {
          if (backlinkHovering) {
              ...
              this.aboutToCloseTimeout = setTimeout(() => {
                  if (!this.backlinkHovering) { // if we're still not hoving on it
